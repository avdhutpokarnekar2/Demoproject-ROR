class WishlistController < ApplicationController
  ...
  def create
    ...
    if product.user_wish_lists.present? && current_user.user_wish_lists.present?
      ...
      wishlist = UserWishList.create(user_id: current_user.id, product_id: product.id)
      redirect_to root_path, notice: 'Product added successfully in wishlist.' if wishlist.save
    end
  end
